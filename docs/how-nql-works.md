...
NQL — SQL for nested data separates boundary formats from a neutral hierarchy,
...
Queries are ordinary H2 SQL plus NQL mapping clauses. Each `into {path}` assigns
...
absent, NQL can infer identity from primary/unique keys, relationships, naming,
and query shape.
...
flowchart LR
    ...
    B --> C[Neutral hierarchy]
    D[NQL DML script] --> E[Path/value mapper]
    C --> E
...
- **Temporary:** a query plus input file builds a fresh H2 database for that
  execution.
- **Persistent:** `cache load` stores H2 files directly under `<cache-dir>` and
  activates the new logical cache. `--cache [--name <name>]` selects an existing
  cache for a run without automatically loading supplied data.
