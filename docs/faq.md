...
## What is NQL's distinctive job?

NQL maps between hierarchical documents and relational data. It can discover
...
expression model. NQL is aimed at jobs that are naturally relational—joins,
...
large tabular, nested, lake, or cloud datasets. NQL’s focus is a neutral
...
over files is not NQL's unique feature. Choose NQL when joined rows need explicit
...
cache without requiring a server. It also matches NQL’s JDBC execution model.
...
## How large a file can NQL handle?
...
state before loading H2, so NQL is not yet presented as a bounded-memory stream
processor. Test a representative file and read [Performance and limits](performance.md).

## Does my data leave the machine?

No data or usage telemetry is sent to the NQL project. NQL connects to a network
...
database use that database’s JDBC driver and dialect. NQL adds mapping clauses
...
Use [GitHub Discussions](https://github.com/blater/nql/discussions/categories/q-a)
for usage questions and sanitized examples. Use an issue for reproducible bugs.
Report vulnerabilities privately through
[GitHub's security advisory form](https://github.com/blater/nql/security/advisories/new).
